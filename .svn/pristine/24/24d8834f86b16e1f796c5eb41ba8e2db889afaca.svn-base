...
(function () {
	var w2grid = function(options) {
		...
		this.last = {
			field		: 'all',
			caption		: w2utils.lang('Todos campos'),
			logic		: 'OR',
			search		: '',
			searchIds 	: [],
			multi		: false,
	...
	w2grid.prototype = {
		...
		buttons: {
			...
			'search-go'	: { type: 'check',  id: 'search-advanced', caption: 'Pesquisar', hint: 'Pesquisar por campos' },
			'add'		: { type: 'button', id: 'add', caption: 'Add New', hint: 'Adicionar novo', img: 'icon-add' },
			'edit'		: { type: 'button', id: 'edit', caption: 'Editar', hint: 'Editar', img: 'icon-edit', disabled: true },
			'delete'	: { type: 'button', id: 'delete', caption: 'Deletar', hint: 'Deletar', img: 'icon-delete', disabled: true },
			'save'		: { type: 'button', id: 'save', caption: 'Salvar', hint: 'Salvar', img: 'icon-save' }
		},
		...
		localSearch: function (silent) {
			...
			time = (new Date()).getTime() - time;
			if (silent !== true) setTimeout(function () { obj.status('Tempo da pesquisa: ' + time/1000 + ' segundos'); }, 10);
			return time;
		},
		...
		searchReset: function (noRefresh) {
			...
			if (this.last.multi) {
				if (!this.multiSearch) {
					...
					this.last.field  	= 'all';
					this.last.caption 	= w2utils.lang('Todos campos');
				}
			}
		...
		refresh: function () {
			...
			if (this.last.multi) {
				searchEl.attr('placeholder', '[' + w2utils.lang('Vários campos') + ']');
			} else {
		...
		initColumnOnOff: function () {
			...
			col_html +=	'<tr><td colspan="2" onclick="w2ui[\''+ obj.name +'\'].columnOnOff(this, event, \'line-numbers\');">'+
						'	<div style="cursor: pointer; padding: 4px 8px; cursor: default">'+ w2utils.lang('Número de linhas') +'</div>'+
						'</td></tr>'+
						'<tr><td colspan="2" onclick="w2ui[\''+ obj.name +'\'].columnOnOff(this, event, \'resize\');">'+
						'	<div style="cursor: pointer; padding: 4px 8px; cursor: default">'+ w2utils.lang('Resetar colunas') + '</div>'+
						'</td></tr>';
			col_html += "</table></div>";
		...
		getSearchesHTML: function () {
			...
			for (var i = 0; i < this.searches.length; i++) {
				...
				if (s.type == 'text') {
					var operator =  '<select id="grid_'+ this.name +'_operator_'+ i +'">'+
						'	<option value="is">'+ w2utils.lang('igual') +'</option>'+
						'	<option value="begins with">'+ w2utils.lang('começa com') +'</option>'+
						'	<option value="contains">'+ w2utils.lang('contém') +'</option>'+
						'	<option value="ends with">'+ w2utils.lang('termina com') +'</option>'+
						'</select>';
				}
				if (s.type == 'int' || s.type == 'float' || s.type == 'date') {
					var operator =  '<select id="grid_'+ this.name +'_operator_'+ i +'" '+
						...
						'			">'+
						'	<option value="is">'+ w2utils.lang('igual') +'</option>'+
						(s.type == 'date' ? '' : '<option value="in">'+ w2utils.lang('em') +'</option>')+
						'	<option value="between">'+ w2utils.lang('entre') +'</option>'+
						'</select>';
				}
				if (s.type == 'list') {
			...
			html += '<tr>'+
					...
					'		<input type="button" onclick="obj = w2ui[\''+ this.name +'\']; if (obj) { obj.search(); }" value="'+ w2utils.lang('Procurar') + '">'+
					'		</div>'+
					'	</td>'+
					'</tr></table>';
			return html;
		...
		scroll: function (event) {
			...
			var url = (typeof this.url != 'object' ? this.url : this.url.get);
			$('#grid_'+ this.name + '_footer .w2ui-footer-right').html(w2utils.formatNumber(this.offset + t1) + '-' + w2utils.formatNumber(this.offset + t2) + ' ' + w2utils.lang('de') + ' ' +	w2utils.formatNumber(this.total) + 
		...
		status: function (msg) {
			if (typeof msg != 'undefined') {
				...
				if (sel.length > 0) {
					...
					if (sel.length == 1) msgLeft = w2utils.lang('Código') + ': '+ tmp + ' ';
				}
